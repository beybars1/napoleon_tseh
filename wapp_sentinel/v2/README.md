...
Napoleon Tseh WhatsApp Sentinel - это комплексная система для автоматизации приема, обработки и учета заказов кондитерской через WhatsApp мессенджер. Система использует искусственный интеллект (OpenAI GPT-4o-mini + LangGraph) для разговорного приема заказов и обработки исторических данных, автоматически генерирует ежедневные отчеты.
...
- **Двуязычная поддержка** — русский и казахский (автоопределение языка)
- **Вежливая форма** — обращение к клиенту на «Вы»
- Контекстное ведение диалога с клиентом
- Пошаговый сбор информации:
  - 📦 Позиции заказа (название, количество/вес)
  - 📅 Дата получения (в т.ч. «завтра», «следующая суббота», «келесі сенбі»)
  - 🕐 Время получения (в т.ч. «на вечер», «утром», «кешке»)
  - 💳 Способ оплаты (предоплата / наличные)
  - 👤 Контактная информация (имя, телефон)
- **Умный разбор дат** — LLM + календарная верификация + валидация дня недели
- **Строгий шаблон меню** — единый формат из БД, без LLM-переформатирования
- Подтверждение и модификация заказа перед сохранением
- Редактирование подтверждённого заказа (окно 2 часа)
...
│   │   ├── order_graph.py         # LangGraph workflow + router
│   │   ├── nodes/                 # LangGraph узлы
│   │   │   ├── intent_classifier.py    # Классификация намерений (GPT-4o-mini)
│   │   │   ├── greeting_node.py        # Приветствие (LLM, двуязычное)
│   │   │   ├── product_inquiry_node.py # Меню и вопросы о продуктах
│   │   │   ├── order_collector_node.py # Сбор деталей заказа
│   │   │   ├── confirmation_node.py    # Подтверждение/модификация заказа
│   │   │   ├── acknowledgment_node.py  # Ack-ответы ("ок", "понятно")
│   │   │   ├── escalation_node.py      # Переключение на оператора
│   │   │   ├── info_provider_node.py   # Вопросы о доставке/оплате
│   │   │   └── small_talk_node.py      # Непрофильные вопросы
│   │   └── tools/                 # LangGraph инструменты
│   │       ├── order_tools.py          # Валидация, разбор дат (LLM), форматирование
│   │       ├── product_tools.py        # Каталог продуктов, шаблон меню
│   │       └── escalation_tools.py     # Утилиты эскалации
│   ├── database/
...
Каждое входящее сообщение обрабатывается за один проход графа:

```
Сообщение → router_node → [specialized_node] → END
```

**Router Node** — классифицирует intent через GPT-4o-mini и маршрутизирует к нужному узлу.

### Узлы обработки

| Узел | Назначение | LLM? |
|------|-----------|------|
| `greeting_node` | Приветствие, краткое представление | ✅ GPT-4o-mini |
| `product_inquiry_node` | Меню (строгий шаблон) / вопросы о продуктах | Шаблон + LLM fallback |
| `order_collector_node` | Извлечение данных заказа, валидация, прогресс | ✅ GPT-4o-mini |
| `confirmation_node` | Подтверждение, модификация, отклонение | ✅ GPT-4o-mini |
| `acknowledgment_node` | «Ок», «понятно» — контекстный ответ | ❌ Шаблон |
| `info_provider_node` | Вопросы о доставке, оплате | ✅ GPT-4o-mini |
| `small_talk_node` | Непрофильные вопросы | ✅ GPT-4o-mini |
| `escalation_node` | Переключение на оператора | ❌ Шаблон |

### Двуязычность (RU/KZ)

- **Автоопределение языка** — по символам қазақ тілі (ә, і, ң, ғ, ү, ұ, қ, ө, һ) и ключевым словам
- **Ответ на языке клиента** — все узлы генерируют ответ на определённом языке
- **Меню** — строгий шаблон `format_menu_for_user()` на обоих языках
- **Вежливая форма** — обращение на «Вы» / «Сіз» во всех ответах

### Умный разбор дат (`resolve_natural_date`)

Вместо простого regex используется двухэтапный подход:

1. **Regex** — для стандартных форматов (`25.01.2026`, `25.01`)
2. **LLM + календарь** — для естественного языка:
   - Генерируется таблица из 15 дней с конкретными датами и пометками «эта неделя» / «СЛЕДУЮЩАЯ неделя»
   - LLM выбирает дату из таблицы (не вычисляет)
   - **Верификация дня недели** — если пользователь упомянул конкретный день, проверяется совпадение и при ошибке корректируется

Поддерживаемые выражения:
- «завтра», «послезавтра», «ертең», «бүрсігүні»
- «следующая суббота», «келесі сенбі»
- «через 3 дня», «3 күннен кейін»
- «на вечер» → 18:00, «утром» → 10:00, «кешке» → 18:00
...
### AI & ML
- **OpenAI GPT-4o-mini** — все LLM-вызовы (интенты, извлечение данных, разбор дат, генерация ответов)
- **LangGraph** — граф состояний для AI агента (single-pass per message)
